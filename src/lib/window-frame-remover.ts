...
var _this: WindowFrameRemover,
    rootPath: string,
    modalDiv: HTMLElement,
    modal: AtomCore.Panel;
...
export default class WindowFrameRemover {
    ...
    public run(): void {
        ...
        if (operation == Operations.CANCEL) return;
    ...
    private styleEndCard(): void {
        ...
        const sheet: CSSStyleSheet = this.getStyleSheet();
    ...
    private deleteFolderRecursive(rootPath: string): void {
        if( fs.existsSync(rootPath) ) {
            fs.readdirSync(rootPath).forEach(function(file: string) {
    ...
    private copyFolderRecursiveSync( source: string, target: string ): void {
        ...
        var traverse = function(folder: string): void {
    ...
    private numFilesSync(source: string): number {
        ...
        var traverse = function(folder: string): void {
            ...
            files.forEach(function(file: string) {
                count++;
                var curSource: string = path.join(source + folder, file);
                if (fs.lstatSync(curSource).isDirectory()) {
                    traverse(folder + "/" + file); // Recurse
                }
            });
    ...
    private removeFrame(step: number | string, exists?: any): void {
        ...
        switch (step) {
            ...
            case 1:
                ...
                else this.removeFrame(2, exists);
                break;
            ...
            case 2:
                ...
                else this.removeFrame(3, exists);
                break;
            ...
            case 3:
                ...
                else this.removeFrame(4, exists);
                break;
            ...
            case 9:
                atom.confirm({
                    message:"Success!",
                    detailedMessage:
                        "The frame removal process completed successfully! However, 'app.asar' cannot be modified while atom is running. "
            ...
            case "revert-guide":
                ...
                setTimeout(function() {
                    atom.confirm({
                        message:"Window Frame Remover Utility",
                        detailedMessage:
                            "'app.asar' cannot be modified while Atom is running. Upon closing this prompt a file-explorer window will open revealing Atom's 'resources' folder. To restore Atom, close the program, delete 'app.asar', and rename 'app.asar.bak' to 'app.asar'.",
                        buttons:{
                            'I Understand': function() { _this.openResDir(); }
                    }
                });
