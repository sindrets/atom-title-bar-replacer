...
function unfocusActiveEditor() {
    var editor = atom.workspace.getActiveTextEditor().element;
	editor.blur();
}
...
function initMenuBar() {
    ...
    $("atom-workspace").keydown(function(e) {
        ...
        else if (e.which == 18 && !(e.altKey && e.ctrlKey) && e.originalEvent.repeat == false) { //alt, disable altGraph
            altOn = true;
        ...
        if ($(".custom-title-bar .menu-label.open").length > 0) {
            ...
            var selectFirst = function() {
                if ($(".menu-label.open .menu-item.selected").length == 0) {
                    target = $(".menu-label.open .menu-item").first();
            ...
            target = $(".menu-label.open .menu-item.selected").last();
