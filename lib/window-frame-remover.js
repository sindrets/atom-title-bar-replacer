...
export default class WindowFrameRemover {
    ...
    run() {
        ...
        atom.confirm({
            message:"Window Frame Remover Utility",
            detailedMessage:"This utility will edit one line in Atom's source (atom-window.js). The modification will allow Atom to start without the native title bar so that the 'Title Bar Replacer' package can serve as the only title bar. Do you wish to proceed?",
            buttons:{
                'Yes': function() { cancel = false },
        ...
        var exists = { 0: false };
        if (fs.existsSync(path+"atom-window.js.bak")) exists[0] = true;
